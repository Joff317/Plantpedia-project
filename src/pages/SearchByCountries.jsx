...
export default function SearchByCountries() {
  ...
  return (
    <div className="min-h-full pt-4 w-full flex flex-col items-center justify-center">
      <h1 className="text-black text-4xl mb-8 font-dancing-script">
        ...
        {countries &&
          countries.map((country) => (
            <Link to={`/searchCountries/${country.slug}`} key={country.id}>
              <p className="text-black bg-white p-2 m-2 rounded-lg cursor-pointer hover:bg-slate-300 text-center">
                {country.name}
              </p>
            </Link>
          ))}
